...
import {
  ...
  ErrorProp,
  OnChangeHandlerFn,
} from "@/pages/feedback-form/[feedback]";
...
export default function CreateAnswerFormat({
  sectionId,
  questionId,
  question,
  answerFormat,
  ...
  switch (type) {
    case "checkbox":
      return (
        <>
          <Question error={error} question={question} required={required} />
          <CheckboxInput
            error={error}
            text={question}
    ...
    case "starrating":
      return (
        <>
          <Question error={error} question={question} required={required} />
          <StarRating count={upperBound ?? 5} onChange={onChangeHandler} />
    ...
    case "emojirating":
      return (
        <>
          <Question error={error} question={question} required={required} />
          <EmojiRating count={upperBound ?? 5} onChange={onChangeHandler} />
          {error && <p className="text-red-600 ml-1 mt-2">{error}</p>}
        </>
      );
    case "textarea":
      return (
        <>
          <Question error={error} question={question} required={required} />
          <MultilineInput minRows={1} onChange={onChangeHandler} />
          {error && <p className="text-red-600 ml-1 mt-2">{error}</p>}
        </>
      );
    case "radio":
      return (
        <>
          <Question error={error} question={question} required={required} />
          <RadioInput options={options ?? []} onChange={onChangeHandler} />
          {error && <p className="text-red-600 ml-1 mt-2">{error}</p>}
        </>
      );
    case "file":
    ...
    case "numberrating":
      return (
        <>
          <Question error={error} question={question} required={required} />
          <NumberRating count={upperBound ?? 5} onChange={onChangeHandler} />
          {error && <p className="text-red-600 ml-1 mt-2">{error}</p>}
        </>
      );
    default:
